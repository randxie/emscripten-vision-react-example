...
vector<Point> findCorners(const vector<Point> &contour) {
  ...
  for_each(contour.cbegin(), contour.cend(), [&](const Point &p) {
...
void applyWarpPerspective(const Mat &matIn, Mat &matOut,
  ...
  auto unwarpedSize =
      Size(max(widthTop, widthBottom), max(heightLeft, heightRight));
  ...
  auto transform =
      getPerspectiveTransform(warpedCorners.data(), unwarpedCorners.data());
...
int *processImage(uchar *array, int width, int height) {
  ...
  auto blockSize = 19;
  auto C = 3;
  adaptiveThreshold(matBinary, matBinary, maxValue, adaptiveMethod,
  ...
  transform(contours.cbegin(), contours.cend(), areas.begin(),
  ...
  int return_image_1_size =
      return_image_1_width * return_image_1_height * return_image_1_channels;
  ...
  int return_image_2_size =
      return_image_2_width * return_image_2_height * return_image_2_channels;

  int numContourPoints = contour.size();
  int return_contour_size = numContourPoints * 2 * sizeof(int);

  int return_data_size = 22 * sizeof(int) + return_image_1_size +
                         return_image_2_size + return_contour_size;
  ...
  int *return_contour_addr =
      reinterpret_cast<int *>(return_image_2_addr + return_image_2_size);
